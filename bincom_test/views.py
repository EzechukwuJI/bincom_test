...
from bincom_test.models import * # import all models in file
...
def lgaSummaryView(request):
	...
	if request.method == "POST":
		...
		for unit in polling_units:
			lga_score_dict[unit] = AnnouncedPuResults.objects.filter(polling_unit_uniqueid = unit.uniqueid) # fet scores for each units
		...
		for party in party_list:
			...
			party_total_score = AnnouncedPuResults.objects.filter(party_abbreviation = party_abbrev, 
				polling_unit_uniqueid__in = polling_units.values('uniqueid')).aggregate(Sum('party_score'))['party_score__sum']

			party_summary_list.append(party_total_score)
		context.update({'lga_score_dict':lga_score_dict,'lga_name': lga_name, 'party_summary_list':party_summary_list})
	...
	if request.method == "POST":
		...
		for key in rp:
			if key not in exclusion_list and rp[key] != "":
				...
				party_score = rp[key],
				entered_by_user = "annonymous",user_ip_address="168.192.4.55" )
